...
class SuffixArray():
    ...
    def __init__(self, document: str):
        ...
        #what if I put self in front of lst
        self.lst = []
        self.indexes = list(range(0, len(document)))
        cmp = lambda x,y:  0 if x.lower() == y.lower() else (-1 if x.lower() < y.lower() else 1)

        for i in range(0, len(document)):
            self.lst.append(document[i:])

        cmpsuffix = lambda x,y:  0 if self.lst[x] == self.lst[y] else (-1 if self.lst[x] < self.lst[y] else 1)

        mysort(self.indexes, cmpsuffix)
        #print(self.indexes)
        #print(self.lst)
    ...
    def positions(self, searchstr: str):
        ...
        strcmp = lambda x,y: 0 if self.lst[x][:len(y)] == y else (-1 if self.lst[x][:len(y)] < y else 1)

        if mybinsearch(self.indexes, searchstr, strcmp) != -1:
            
            return[mybinsearch(self.indexes, searchstr, strcmp) - 1]
    ...
    def contains(self, searchstr: str):
        ...
        testing = mybinsearch(self.indexes, searchstr, strcmp)
